...
for (let i=0;i<s.length;i++){
  cvs+=\`<div class="sepia-contrast p-1"><p class="zh" onclick="playAudio('\${s[i].sentence_pron_file}')"><strong>\${i+1}. \${s[i].orgnc_sentence}<br>\${s[i].trsl_orgnc_sentence}</strong></p></div><div class="fs12">\${gpt[i]}</div><br><hr>\`;
}
document.getElementById('cvs').innerHTML=cvs;

var e=data.studys[0].examples;
for (let j=0;j<e.length;j++){
  std+=\`<div class="sepia-contrast p-1"><p class="zh" onclick="playAudio('\${e[j].pron_file_url}')"><strong>\${j+1}. \${e[j].origin_example}<br>\${e[j].origin_translation}</strong></p></div><div class="fs12">\${gpt[s.length+j]}</div><br><hr>\`;
}
